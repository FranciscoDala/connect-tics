...
UPLOAD_DIR = STATIC_DIR / "assets" / "img" / "uploads"
UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
BASE_URL_PATH = "/static/assets/img/uploads"

logger.info(f"PROJECT_ROOT: {PROJECT_ROOT.resolve()}")
logger.info(f"UPLOAD_DIR FIXO: {UPLOAD_DIR.resolve()}")
...
def _build_post_out(post: PostModel, category_name: str) -> dict:
    return {
        "id": post.id, "category_id": post.category_id,
        "category_name": category_name or "Sem Categoria", "title": post.title,
        "type": post.type.value if isinstance(post.type, enum.Enum) else post.type,
        "is_highlighted": post.is_highlighted, "content": post.content,
        "price": post.price, "link": post.link, "cover_image": post.cover_image,
...
def read_posts(limit: int = 100, skip: int = 0, db: Session = Depends(get_db)):
    stmt = select(PostModel).options(joinedload(PostModel.images), joinedload(PostModel.category)).order_by(PostModel.created_at.desc()).offset(skip).limit(limit)
...
async def create_post(
    category_id: int = Form(...), title: str = Form(...), type: PostType = Form(...),
    is_highlighted: bool = Form(False), content: str | None = Form(None),
    price: float | None = Form(None), link: str | None = Form(None),
    cover_image: UploadFile | None = File(None), db: Session = Depends(get_db)
):
    ...
    if type == PostType.fotos and not content: content = "" # Evita null no banco

    cover_url = _save_upload(cover_image)
    db_post = PostModel(category_id=category_id, title=title, type=type, is_highlighted=is_highlighted, content=content, price=price, link=link, cover_image=cover_url)
    db.add(db_post); db.commit(); db.refresh(db_post)
    return _build_post_out(db_post, category.name)

@router.post("/{post_id}/images")
async def upload_post_image(post_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
    db_post = db.get(PostModel, post_id)
    if not db_post: raise HTTPException(status_code=404, detail="Post not found")
    if db_post.type!= PostType.fotos: raise HTTPException(status_code=400, detail="Só posts do tipo 'fotos' aceitam multiplas imagens")
    url = _save_upload(file)
    if not url: raise HTTPException(status_code=400, detail="Arquivo inválido")
    db.add(PostImageModel(post_id=post_id, image_url=url)); db.commit()
...
async def update_post(
    post_id: int, category_id: int = Form(...), title: str = Form(...), type: PostType = Form(...),
    is_highlighted: bool = Form(False), content: str | None = Form(None),
    price: float | None = Form(None), link: str | None = Form(None),
    cover_image: UploadFile | None = File(None), existing_images: list[str] = Form([]),
    ...
    db_post.category_id = category_id; db_post.title = title; db_post.type = type
    db_post.is_highlighted = is_highlighted; db_post.content = content or ""
    db_post.price = price; db_post.link = link
    ...
    db.commit(); db.refresh(db_post, attribute_names=['images'])
    return _build_post_out(db_post, category.name)
